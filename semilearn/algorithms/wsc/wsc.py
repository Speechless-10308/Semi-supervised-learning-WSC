...
class WSC(AlgorithmBase):
    ...
    def set_model(self): 
        ...
        model = WSC_Net(model, proj_size=self.args.proj_size, epass=self.args.use_epass)
        return model
    ...
    def train_step(self, x_lb_w, x_lb_s_0, x_lb_s_1, y_lb, x_ulb_w, x_ulb_s_0, x_ulb_s_1):
        ...
        with self.amp_cm():
            if self.use_cat:
                ...
                feats_x_lb_w, feat_x_lb_s_0, feat_x_lb_s_1 = outputs['feat'][:num_lb*3]
            ...
            wsc_feat_dict, wsc_q = self.call_hook("get_feats", "WSCFilterHook", feat_dict=feat_dict, y_lb=y_lb, pseudo_label=pseudo_label, mask=mask)

            wsc_loss = self.weak_spectral_loss(
